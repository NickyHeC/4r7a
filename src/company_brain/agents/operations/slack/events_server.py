"""Slack Events API listeners — Socket Mode (local) and HTTP (cloud)."""
...
def serve_events(*, http: bool = False, host: str = "0.0.0.0", port: int = 3000) -> None:
    config = load_config()
    if http or cfg.events_mode() == "http":
...
def _serve_socket(config: AppConfig) -> None:
    ...
    if not slack_client.socket_mode_configured():
        raise RuntimeError("SLACK_WIKI_BOT_TOKEN and SLACK_WIKI_APP_TOKEN required for Socket Mode")

    router = SlackEventsRouter(config)
    join_stats = slack_client.join_internal_channels()
    logger.info("Joined internal channels: %s", join_stats)
    ...
    def _process(client: SocketModeClient, req: SocketModeRequest) -> None:
        ...
        logger.debug("Event result: %s", result)

    client = SocketModeClient(
        app_token=slack_client.wiki_app_token(),
        web_client=slack_client.client(),
    )
    client.socket_mode_request_listeners.append(_process)
    logger.info("Slack wiki events listening (Socket Mode)")
    client.connect()
    try:
...
def _serve_http(config: AppConfig, *, host: str, port: int) -> None:
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_POST(self) -> None:  # noqa: N802
            ...
            if not slack_client.verify_http_signature(body, timestamp, signature):
            ...
            logger.debug("HTTP event result: %s", result)
    ...
    join_stats = slack_client.join_internal_channels()
    logger.info("Joined internal channels: %s", join_stats)
    logger.info("Slack wiki events listening (HTTP %s:%s%s)", host, port, path)
