...
ephemeral smol VM (https://github.com/smol-machines/smolvm) with the wiki (or a
copy of it) mounted at ``/workspace`` and egress locked to an allow-list.
...
class SmolSandbox:
    """Runs a command in an ephemeral smol VM with a mounted directory."""

    def __init__(self, image: str = DEFAULT_IMAGE):
        self.image = image

    def available(self) -> bool:
        return resolve_sandbox() == "smolvm" and shutil.which("smolvm") is not None
...
def verify_in_sandbox(
    ...
    *,
    ...
    """Run a verification command in a smol sandbox.
